...
uint32_t Communicator::getMasterSockfd() {
    // TODO: assume return first MASTER
    map<uint32_t, Connection*>::iterator p;

    boost::shared_lock<boost::shared_mutex> lock(connectionMapMutex);

    for (p = _connectionMap.begin(); p != _connectionMap.end(); p++) {
        if (p->second->getConnectionType() == MASTER) {
...
uint32_t Communicator::getChunkserverSockfd() {
    // TODO: assume return first Chunkserver
    map<uint32_t, Connection*>::iterator p;

    boost::shared_lock<boost::shared_mutex> lock(connectionMapMutex);

    for (p = _connectionMap.begin(); p != _connectionMap.end(); p++) {
        if (p->second->getConnectionType() == CHUNKSERVER) {
            return p->second->getSockfd();
        }
    }
...
vector<struct Component> Communicator::parseConfigFile(string componentType) {
    ...
    for (uint32_t i = 0; i < componentCount; i++) {
        ...
        if (componentType == "MASTER")
            component.type = MASTER;
        else if (componentType == "CHUNKSERVER")
            component.type = CHUNKSERVER;
...
void Communicator::connectToComponents(vector<Component> componentList) {
    ...
    for (Component component : componentList) {
        ...
        } else if ((_componentType == MASTER || _componentType == CHUNKSERVER)
                && component.type == MONITOR) {
            debug("Connecting to %s:%" PRIu16 "\n", component.ip.c_str(),
                    component.port);
            uint32_t sockfd = connectAndAdd(component.ip, component.port,
...
void Communicator::connectToMaster() {
    vector<Component> masterList = parseConfigFile("MASTER");
    printComponents("MASTER", masterList);
    for (Component component : masterList) {
        uint32_t sockfd = connectAndAdd(component.ip, component.port,
                component.type);
        _sockfdBufMap[sockfd] = new RecvBuffer();
        debug_cyan(
                "connectToMaster: Add socket to mutex and buf map %" PRIu32 "\n",
                sockfd);
        requestHandshake(sockfd, _componentId, _componentType);
    }
}

void Communicator::connectToChunkserver(uint32_t dstChunkserverIp, uint32_t dstChunkserverPort) {
    uint32_t sockfd = connectAndAdd(Ipv4Int2Str(dstChunkserverIp), dstChunkserverPort,
            _componentType);
    _sockfdBufMap[sockfd] = new RecvBuffer();
    debug_cyan("connectToChunkserver: Add socket to mutex and buf map %" PRIu32 "\n",
            sockfd);
    requestHandshake(sockfd, _componentId, _componentType);
}
...
void Communicator::connectAllComponents() {
    ...
    // parse config file
    vector<Component> masterList = parseConfigFile("MASTER");
    vector<Component> chunkserverList = parseConfigFile("CHUNKSERVER");
    vector<Component> monitorList = parseConfigFile("MONITOR");

    // debug
    printComponents("MASTER", masterList);
    printComponents("CHUNKSERVER", chunkserverList);
    printComponents("MONITOR", monitorList);

    // connect to components
    connectToComponents(masterList);
    connectToComponents(chunkserverList);
...
uint32_t Communicator::sendSegment(uint32_t componentId, uint32_t sockfd,
...
#ifdef COMPILE_FOR_CHUNKSERVER
    updateKey = to_string(chunkserver->getChunkserverId()) + "." + to_string(_updateId++);
#endif
    ...
    uint32_t ifrom = segmentData.info.ifrom;
    ...
    if (segmentData.bufLength <= _chunkSize) {
        return putSmallSegment(componentId, sockfd, segmentId,
                ...
                codingSetting, updateKey, buf, offsetLength,ifrom);
    }
...
DataMsgType Communicator::putSegmentInit(uint32_t componentId,
        uint32_t dstChunkserverSockfd, uint64_t segmentId, uint32_t segLength,
    ...
    PutSegmentInitRequestMsg* putSegmentInitRequestMsg =
            new PutSegmentInitRequestMsg(this, dstChunkserverSockfd, segmentId,
...
void Communicator::putSegmentData(uint32_t componentID, uint32_t dstChunkserverSockfd,
    ...
    SegmentDataMsg* segmentDataMsg = new SegmentDataMsg(this, dstChunkserverSockfd,
...
void Communicator::putSegmentEnd(uint32_t componentId, uint32_t dstChunkserverSockfd,
        uint64_t segmentId, DataMsgType dataMsgType, string updateKey,
        vector<offset_length_t> offsetLength,uint32_t ifrom) {

    // Step 3 of the upload process
    SegmentTransferEndRequestMsg* putSegmentEndRequestMsg =
            new SegmentTransferEndRequestMsg(this, dstChunkserverSockfd, segmentId,
                    dataMsgType, updateKey, offsetLength,ifrom);
...
uint32_t Communicator::putSmallSegment(uint32_t componentId,
        ...
        string updateKey, char* buf, vector<offset_length_t> offsetLength,uint32_t ifrom) {
    printf("\033[44;37;5m putSmallSegment\033[0m\n");
    PutSmallSegmentRequestMsg* putSmallSegmentRequestMsg =
            new PutSmallSegmentRequestMsg(this, dstChunkserverSockfd, segmentId,
                    segLength, bufLength, codingScheme, codingSetting,
                    offsetLength, ifrom,updateKey);

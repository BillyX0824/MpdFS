...
public:
	 ...
	 * @param Destination type: Master/CLIENT/MONITOR/CHUNKSERVER
	...
	// FOR TESTING ONLY
	uint32_t getMasterSockfd();
	uint32_t getMonitorSockfd();
	uint32_t getChunkserverSockfd();

	/**
	 * Obtain a list of Master retrieved from config file
	 * @return list of Master Component
	 */

	vector<Component> getMasterList();

	/**
	 * Obtain a list of CHUNKSERVER retrieved from config file
	 * @return list of CHUNKSERVER Component
	 */

	vector<Component> getChunkserverList();
...
//TODO: documentation
	...
	void connectToChunkserver(uint32_t ip, uint32_t port);
...
protected:
	 ...
	 * @param componentId My Component ID
	 * @param dstChunkserverSockfd Destination CHUNKSERVER Socket Descriptor
	...
	DataMsgType putSegmentInit(uint32_t componentId, uint32_t dstChunkserverSockfd,
	...
	void putSegmentData(uint32_t componentID, uint32_t dstChunkserverSockfd,
	...
	void putSegmentEnd(uint32_t componentId, uint32_t dstChunkserverSockfd,
			uint64_t segmentId, DataMsgType dataMsgType, string updateKey,
			vector<offset_length_t> offsetLength,uint32_t ifrom);

	uint32_t putSmallSegment (uint32_t componentId, uint32_t dstChunkserverSockfd,
	        uint64_t segmentId, uint32_t segLength, uint32_t bufLength,
	...
	// component list
	vector<Component> masterList;
	vector<Component> chunkserverList;

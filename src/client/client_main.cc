...
//	}
	...
	try {
		...
		desc.add_options()
				...
				("ifrom,o", po::value<int>(&ifrom),"if read only")
		...
		try {
			 ...
			 */
			if (vm.count("help")) {
				cout << endl << "UPLOAD: ./CLIENT -i [ID] -a upload -c [coding] -n [stripes] -o [ifrom] [SRC]" << endl;
...
// handle signal for profiler
	...
	*/
	...
	communicator->getChunkserverListAndConnect();

	if (action == "upload") {
		client->uploadFileRequest(file, codingScheme, codingSetting,ifrom);
	} else {

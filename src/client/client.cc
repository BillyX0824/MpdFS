...
void startUploadThread(uint32_t clientId, uint32_t sockfd,
	...
	client->getCommunicator()->sendSegment(clientId, sockfd, segmentData,codingScheme, codingSetting);
...
uint32_t Client::uploadFileRequest(string path, CodingScheme codingScheme,
		string codingSetting,uint32_t ifrom) {
	...
	for (uint32_t i = 0; i < segmentCount; ++i) {
		...
		_tp.schedule(
				boost::bind(startUploadThread, _clientId, dstChunkserverSockfd,
...
void Client::downloadFileRequest(uint32_t fileId, string dstPath) {
	...
	// 1. Get file infomation from Master
	...
	// 2. Download file from CHUNKSERVER

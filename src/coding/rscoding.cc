...
block_list_t RSCoding::getRequiredBlockSymbols(vector<bool> blockStatus,
	...
	// if more than one in secondaryChunkserverStatus is false, return {} (error)
	int failedChunkserverCount = (int) count(blockStatus.begin(), blockStatus.end(),
			false);
	...
	if ((uint32_t) failedChunkserverCount > m) {
		return {};
	}
...
int* RSCoding::getMatrix(string setting) {
	vector<uint32_t> params = getParameters(setting);
	const uint32_t k = params[0];
	const uint32_t m = params[1];
	const uint32_t w = params[2];
...
uint32_t RSCoding::getWNumber(string setting) {
	vector<uint32_t> params = getParameters(setting);
	const uint32_t w = params[2];
	return w;
}
...
int RSCoding::getgame(int *matrix,int blockID,int k ,int parityId) {
	int game;
	int i = parityId-k;
...
vector<BlockData> RSCoding::computeDelta(BlockData oldBlock, BlockData newBlock,
	...
	const uint32_t k = params[0];
	int *matrix = getMatrix(codingSetting);
	vector<BlockData> deltaS (parityVector.size());
	BlockData DataDelta;
	DataDelta.buf = MemoryPool::getInstance().poolMalloc(combinedLength);
	bitwiseXor(DataDelta.buf, oldBlock.buf, newBlock.buf, combinedLength);
	for (int i = 0; i < (int) deltaS.size(); i++) {
		BlockData &delta = deltaS[i];											 //to get parity delta
		delta.info = oldBlock.info;
		delta.info.blockIdOfDelta = oldBlock.info.blockId; // get according blockId of delta
		delta.buf = MemoryPool::getInstance().poolMalloc(combinedLength);
		int game = getgame(matrix,delta.info.blockIdOfDelta,k,k+i);
		galois_w08_region_multiply(DataDelta.buf, game,combinedLength, delta.buf, 0);
	}
    return deltaS;
}

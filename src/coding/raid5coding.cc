...
block_list_t Raid5Coding::getRequiredBlockSymbols(vector<bool> blockStatus,
	...
	// no CHUNKSERVER failure / parity CHUNKSERVER failure
	uint32_t blockRange = 0;

	if (failedChunkserverCount == 0
			|| (failedChunkserverCount == 1 && blockStatus.back() == false)) {
...
int*  Raid5Coding::getMatrix(string setting) {
}
uint32_t Raid5Coding::getWNumber(string setting){
}

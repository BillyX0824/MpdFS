...
vector<uint64_t> MetaDataModule::readChunkserverPrimarySegmentList(uint32_t chunkserverId) {
	return _segmentMetaDataModule->findChunkserverPrimarySegments(chunkserverId);
}

vector<uint64_t> MetaDataModule::readChunkserverSegmentList(uint32_t chunkserverId) {
	return _segmentMetaDataModule->findChunkserverSegments(chunkserverId);
}
 ...
 */
void MetaDataModule::setPrimary(uint64_t segmentId, uint32_t primaryChunkserverId) {
	_segmentMetaDataModule->setPrimary(segmentId, primaryChunkserverId);
	return;
}
...
uint32_t MetaDataModule::selectActingPrimary(uint64_t segmentId,
	...
	int failedChunkserverCount = (int) count(nodeStatus.begin(), nodeStatus.end(),
			false);

	if (failedChunkserverCount == (int) nodeStatus.size()) {
		debug_yellow("All CHUNKSERVER has died for segmentId = %" PRIu64 "\n",
				segmentId);
		exit(-1);
	}
	...
	// random until a healthy CHUNKSERVER is found
	while (true) {

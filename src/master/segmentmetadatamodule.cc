...
vector<uint64_t> SegmentMetaDataModule::findChunkserverSegments(uint32_t chunkserverId) {
	vector<uint64_t> segmentList;
	BSONObj querySegment = BSON ("nodeList" << (int) chunkserverId);
...
vector<uint64_t> SegmentMetaDataModule::findChunkserverPrimarySegments(uint32_t chunkserverId) {
	vector<uint64_t> segmentList;
	BSONObj querySegment = BSON ("primary" << (int) chunkserverId);
	vector<BSONObj> result = _segmentMetaDataStorage->read(querySegment);
	for (auto bson : result) {
		uint64_t id = (uint64_t) bson.getField("id").numberLong();
		segmentList.push_back(id);

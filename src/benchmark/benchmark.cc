...
void testDownload() {
	...
	if (hotness) {
		...
		for (int i : requestList) {
			...
			uint32_t dstChunkserverSockfd = _clientCommunicator->getSockfdFromId(primary);
			uint64_t segmentId = fileMetaData._segmentList[i];
			debug("segmentId = %" PRIu64 " primary = %" PRIu32 "\n",
					segmentId, primary);

			_tp.schedule(
					boost::bind(startBenchDownloadThread, clientId, dstChunkserverSockfd,
							segmentId));
		}
	} else {
	...
	cout << formatSize(fileMetaData._size) << " transferred in " << duration
			<< " sechunkserver, Rate = " << formatSize(fileMetaData._size / duration)
			<< "/s" << endl;
}
...
void testUpload() {
	...
	map<uint32_t, uint32_t> CHUNKSERVERLoadCount;
	for (uint32_t i = 0; i < numberOfSegment; ++i) {
		...
		if (CHUNKSERVERLoadCount.count(primary) == 0)
			CHUNKSERVERLoadCount[primary] = 1;
		else
			CHUNKSERVERLoadCount[primary] += 1;
		...
		_tp.schedule(
				boost::bind(startBenchUploadThread, clientId, dstChunkserverSockfd,
	...
	cout << formatSize(fileSize) << " transferred in " << duration
			<< " sechunkserver, Rate = " << formatSize(fileSize / duration) << "/s"
			<< endl;

	map<uint32_t, uint32_t>::iterator it;

	for (it = CHUNKSERVERLoadCount.begin(); it != CHUNKSERVERLoadCount.end(); ++it) {

...
void PutSmallSegmentRequestMsg::prepareProtocolMsg() {
    ...
    putSegmentInitRequestPro.set_ifrom(_ifrom);
...
void PutSmallSegmentRequestMsg::parse(char* buf) {
    ...
    _ifrom = putSegmentInitRequestPro.ifrom();
...
#ifdef COMPILE_FOR_CHUNKSERVER
    _dataMsgType = chunkserver->putSegmentInitProcessor(_msgHeader.requestId, _sockfd,
            _segmentId, _segmentSize, _bufferSize, 1, _codingScheme,
            _codingSetting, _updateKey, true);
    chunkserver->putSegmentDataProcessor(_msgHeader.requestId, _sockfd, _segmentId, 0,
            _bufferSize, _dataMsgType, _updateKey, _payload);
    chunkserver->putSegmentEndProcessor(_msgHeader.requestId, _sockfd, _segmentId,
            _dataMsgType, _updateKey, _offsetLength,  _ifrom, true);
#endif

...
SegmentTransferEndRequestMsg::SegmentTransferEndRequestMsg(
		Communicator* communicator, uint32_t chunkserverSockfd, uint64_t segmentId,
...
void SegmentTransferEndRequestMsg::prepareProtocolMsg() {
	...
	segmentTransferEndRequestPro.set_ifrom(_ifrom);
...
void SegmentTransferEndRequestMsg::parse(char* buf) {
	...
	_ifrom = segmentTransferEndRequestPro.ifrom();
...
void SegmentTransferEndRequestMsg::doHandle() {
	...
	chunkserver->putSegmentEndProcessor(_msgHeader.requestId, _sockfd, _segmentId,
			_dataMsgType, _updateKey, _offsetLength,_ifrom);
#endif

...
void BlockTransferEndRequestMsg::prepareProtocolMsg() {
	...
	for (it1 = _parityList.begin(); it1 < _parityList.end(); ++it1) {
        ...
        blockLocationPro->set_chunkserverid((*it1).chunkserverId);
...
void BlockTransferEndRequestMsg::parse(char* buf) {
	...
	for (int i = 0; i < blockTransferEndRequestPro.blocklocation_size(); ++i) {
	        ...
	        tempBlockLocation.chunkserverId =
	                blockTransferEndRequestPro.blocklocation(i).chunkserverid();
...
#ifdef COMPILE_FOR_CHUNKSERVER
	chunkserver->putBlockEndProcessor(_msgHeader.requestId, _sockfd, _segmentId,

 ...
 */
DeleteFileRequestMsg::DeleteFileRequestMsg(Communicator* communicator,
		uint32_t masterSockfd, uint32_t clientId, uint32_t fileId, const string &path) :
		Message(communicator) {
...
#ifdef COMPILE_FOR_MASTER
	master->deleteFileProcessor(_msgHeader.requestId, _sockfd, _clientId, _fileId, _path);
#endif
}

 ...
 */
GetSegmentIdListRequestMsg::GetSegmentIdListRequestMsg(Communicator* communicator, uint32_t masterSockfd, uint32_t clientId, uint32_t numOfObjs) :
		Message(communicator) {
...
#ifdef COMPILE_FOR_MASTER
	master->getSegmentIdListProcessor(_msgHeader.requestId, _sockfd, _clientId, _numOfObjs);
#endif
}

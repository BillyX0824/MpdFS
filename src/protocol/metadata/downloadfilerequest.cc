 ...
 */
DownloadFileRequestMsg::DownloadFileRequestMsg(Communicator* communicator,
		uint32_t masterSockfd, uint32_t clientId, uint32_t fileId) :
		Message(communicator) {
 ...
 */
DownloadFileRequestMsg::DownloadFileRequestMsg(Communicator* communicator,
		uint32_t masterSockfd, uint32_t clientId, const string &filePath) :
		Message(communicator) {
	_sockfd = masterSockfd;
	_clientId = clientId;
	_fileId = 0;
...
#ifdef COMPILE_FOR_MASTER
	if(_fileId == 0)
		master->downloadFileProcessor(_msgHeader.requestId, _sockfd, _clientId, _filePath);
	else
		master->downloadFileProcessor(_msgHeader.requestId, _sockfd, _clientId, _fileId);
#endif
}

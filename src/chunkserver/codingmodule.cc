...
int* CodingModule::getMatrix(CodingScheme codingScheme, string setting){
	return getCoding(codingScheme)->getMatrix(setting);
}
...
uint32_t CodingModule::getWNumber(CodingScheme codingScheme, string setting) {
    return getCoding(codingScheme)->getWNumber(setting);
}
...
vector<BlockData> CodingModule::unpackUpdates(CodingScheme codingScheme, 
    ...
    uint32_t blkSize = getCoding(codingScheme)->getBlockSize(segmentSize, setting);//blocksize
    uint32_t blkN = getCoding(codingScheme)->getBlockCountFromSetting(setting);// k+n number

...
public:
    ...
    BlockData getBlock(uint64_t segmentId, uint32_t blockId, bool isParity,DataMsgType dataMsgType,
    ...
    BlockData getMergedBlock(uint64_t segmentId, uint32_t blockId,
            bool isParity, bool needLock, vector<offset_length_t> symbols);
    BlockData getMergedBlockx(uint64_t segmentId, uint32_t blockId,
                             bool isParity, bool needLock, vector<offset_length_t> symbols);
    uint32_t getDeltaTotalNum(uint64_t segmentId, uint32_t blockId,uint32_t getDeltaNum,uint32_t allDeltaNum);
    vector<BlockData> getParityDelta(uint64_t segmentId, uint32_t blockId,
            bool needLock,uint32_t getDeltaNum,uint32_t allDeltaNum);
    ...
    uint32_t writeDeltaBlock(uint64_t segmentId, uint32_t blockId,
            ...
            bool isParity);
    uint32_t writeDeltaBlock(uint64_t segmentId, uint32_t blockId,
                             uint32_t deltaId,uint32_t blockIdOfDelta, char* buf, vector<offset_length_t> offsetLength,
                             bool isParity);
    ...
    int getgame(int *matrix,int blockID,int k,int parityId);
    void flushDeltaBlock(uint64_t segmentId, uint32_t blockId, uint32_t deltaId,
            bool isParity);

    /**
     * Set the Capacity of CHUNKSERVER
     * @param max_block capacity of CHUNKSERVER
     */

    void setMaxBlockCapacity(uint32_t max_block);

    /**
     * Get the Capacity of CHUNKSERVER
     * @return uint32_t Max capacity of CHUNKSERVER
     */

    uint64_t getMaxBlockCapacity();

    /**
     * Get the current Capacity of CHUNKSERVER
     * @return uint32_t current capacity of CHUNKSERVER
     */

    uint64_t getCurrentBlockCapacity();

    /**
     * Get the free space of CHUNKSERVER
     * @return uint32_t current free space of CHUNKSERVER
     */
    void createSegmentCache(uint64_t segmentId,uint32_t ifrom);
    uint32_t  getSegmentCache(uint64_t segmentId);
    uint64_t getFreeBlockSpace();

    /**
     * Verify whether CHUNKSERVER has enough space
...
private:
    ...
    /**
     * Calculate and update the free space and usage of CHUNKSERVER
...
//    DeltaLocation getDeltaLocation (uint64_t segmentId, uint32_t blockId, uint32_t deltaId);
    ...
    map <uint64_t, uint32_t> _segmentCache;
};

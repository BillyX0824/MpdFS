...
uint32_t StorageModule::getNextDeltaId (uint32_t segmentId, uint32_t blockId) {
    ...
    _deltaIdMap.set(blockKey,curDeltaId+1);
    debug("segmentId: %" PRIu32 ",blockID: %" PRIu32 ",curDeltaId = %" PRIu32 "\n", segmentId,blockId,curDeltaId);
    return curDeltaId;
}
...
BlockData StorageModule::getBlock (uint64_t segmentId, uint32_t blockId, bool isParity, DataMsgType dataMsgType, vector<offset_length_t> symbols, bool needLock) {
    if (isParity) {
        if(dataMsgType == RECOVERY){// 3
            if (_updateScheme == FO || _updateScheme == MPLR) { // M-PLR repair only get old block firstly
                return readBlock(segmentId, blockId, symbols);
            } else {
                return getMergedBlockx(segmentId, blockId, isParity, true, symbols);
            }
        }else{
            if (_updateScheme == FO) {
                return readBlock(segmentId, blockId, symbols);
            } else {
                return getMergedBlock(segmentId, blockId, isParity, true, symbols);
            }
        }
    } else {
        if (_updateScheme == FL) {
...
BlockData StorageModule::getMergedBlock (uint64_t segmentId, uint32_t blockId, bool isParity, bool needLock, vector<offset_length_t> symbols) {
    ...
    readFile(blockPath, blockData.buf, 0, blockData.info.blockSize, false);//read BlockData
    ...
    BlockData delta;
    for (DeltaLocation deltaLocation : _deltaLocationMap.get(blockKey)) {//get the blockKey's delta (for m parity)
        const uint32_t deltaId = deltaLocation.deltaId;

        // read from reserved space if delta is inside
        if (deltaLocation.isReserveSpace) {
            debug_red("Reading from Reserve Segment ID = %" PRIu64 " Block ID = %" PRIu32 " Delta ID = %" PRIu32 "\n", segmentId, blockId, deltaId);
            string deltaKey = generateDeltaKey (segmentId, blockId, deltaId);//segmentId.blockId.deltaId
            vector<offset_length_t> offsetLength = _deltaOffsetLength.get(deltaKey);
            //offsetLength[0].first =1;
            uint32_t combinedLength = getCombinedLength(offsetLength);
            delta.info.segmentId = segmentId;
            delta.info.blockId = blockId;
            delta.info.blockSize = combinedLength; // size of delta (total)
            delta.info.offlenVector = offsetLength; //
            delta.buf = MemoryPool::getInstance().poolMalloc(combinedLength);
            memcpy (delta.buf, wholeBuf + deltaLocation.offsetLength.first, combinedLength);//read all space delta in buf
        } else {
            debug_red("Reading from Delta Block Segment ID = %" PRIu64 " Block ID = %" PRIu32 " Delta ID = %" PRIu32 "\n", segmentId, blockId, deltaId);
            delta = readDeltaBlock(segmentId, blockId, deltaId);
        }

        // perform merging
        char* deltaBufPtr = delta.buf;
        for (offset_length_t offsetLengthPair : delta.info.offlenVector) {//for ever delta off-len to merge
            uint32_t offset = offsetLengthPair.first;
            uint32_t length = offsetLengthPair.second;
            debug_red("Merging Delta offset = %" PRIu32 " length = %" PRIu32 "\n", offset, length);
            if (isParity) {
	        printf("Merging PARITY Delta offset = %" PRIu32 " length = %" PRIu32 "\n", offset, length);
                Coding::bitwiseXor(blockData.buf + offset, blockData.buf + offset, deltaBufPtr, length);
            } else {
	        printf("Merging DATA Delta offset = %" PRIu32 " length = %" PRIu32 "\n", offset, length);
    ...
    // if not all symbols are needed, pack according to the desired order
    if ( ! symbols.empty() ) {//symbols:blocks off-len need to get to repair
        memcpy(wholeBuf, blockData.buf, blockData.info.blockSize);// copy new blockData to wholeBuf
        uint32_t symbolOffset = 0;
        for (auto offlen : symbols) {
            memcpy(blockData.buf + symbolOffset, wholeBuf + offlen.first, offlen.second);//pack  the desired order
            symbolOffset += offlen.second;// to next offset
        }
    }
    MemoryPool::getInstance().poolFree(wholeBuf);
    return blockData;
}
BlockData StorageModule::getMergedBlockx (uint64_t segmentId, uint32_t blockId, bool isParity, bool needLock, vector<offset_length_t> symbols) {

    const string blockPath = generateBlockPath(segmentId, blockId, _blockFolder);
    const string blockKey = getBlockKey (segmentId, blockId);
    const string segmentKey = to_string(segmentId);
    RWMutex* rwmutex = obtainRWMutex(blockKey);

    readLock rdlock(*rwmutex, boost::try_to_lock_t());
    RWMutex* rwmutexx = obtainRWMutex(segmentKey);

    readLock rdlockx(*rwmutexx, boost::try_to_lock_t());
    // if not need lock, it means the caller already has lock
    if (needLock && !rdlock.owns_lock() && !rdlockx.owns_lock()) {
        rdlock.lock();
    }
...
uint32_t StorageModule::getDeltaTotalNum(uint64_t segmentId, uint32_t blockId,uint32_t getDeltaNum,uint32_t allDeltaNum){
    uint32_t deltaCount = getDeltaCount(segmentId,blockId);
    uint32_t beginDeltaId;
    uint32_t endDeltaId;
    if (allDeltaNum == 1){
        beginDeltaId = 0;
        endDeltaId = deltaCount-1;
    } else {
        uint32_t readDeltaCount = deltaCount / allDeltaNum;
        uint32_t remainingDelta = deltaCount-readDeltaCount*allDeltaNum;
        if (getDeltaNum == 1){
            beginDeltaId = 0;
            endDeltaId = remainingDelta+readDeltaCount-1;
        }else {
            beginDeltaId = remainingDelta+(getDeltaNum-1)*readDeltaCount;
            endDeltaId = beginDeltaId+readDeltaCount-1;
        }
    }
    uint32_t gettotal = endDeltaId - beginDeltaId + 1;
    debug_red(
                "[DELTA] Need to obtain delta in block %" PRIu32 ",beginDelta:%" PRIu32 ",,endDelta:%" PRIu32 ", deltaCount: %" PRIu32 "\n",
                blockId,beginDeltaId,endDeltaId,gettotal);
    return gettotal;
}
// this function is only thread-safe when needLock == true
vector<BlockData> StorageModule::getParityDelta (uint64_t segmentId, uint32_t blockId, bool needLock,uint32_t getDeltaNum,uint32_t allDeltaNum) {
    vector<BlockData> delta(getDeltaCount(segmentId,blockId));
    const string blockPath = generateBlockPath(segmentId, blockId, _blockFolder);
    const string blockKey = getBlockKey (segmentId, blockId);
    uint32_t deltaCount = getDeltaCount(segmentId,blockId);
    debug_red("[DELTA] segmentId = %" PRIu64 ",blockId = %" PRIu32 ",deltaCount = %" PRIu32"\n", segmentId,blockId,deltaCount);
    uint32_t beginDeltaId;
    uint32_t endDeltaId;
    bool ifNeedGet;
    if (allDeltaNum == 1){
    ...
    if ((endDeltaId - beginDeltaId + 1) == 0){
        ifNeedGet = false;
    }else {
        ifNeedGet = true;
    }
    if (ifNeedGet){
        debug_red(
                "[DELTA] Need to obtain delta in block %" PRIu32 ", beginDelta:%" PRIu32 ", endDelta:%" PRIu32 ", deltaCount: %" PRIu32 "\n",
                blockId,beginDeltaId,endDeltaId,endDeltaId-beginDeltaId+1);
        ...
        //get delta
        uint32_t i = 0;
        vector<int> needDeltaNum(endDeltaId - beginDeltaId + 1);
        for (uint32_t j = beginDeltaId; j < endDeltaId+1; ++j) {
            needDeltaNum[j-beginDeltaId]=j;
        }
        for (DeltaLocation deltaLocation : _deltaLocationMap.get(blockKey)) {//get the blockKey's delta (for m parity)
            const uint32_t deltaId = deltaLocation.deltaId;
            // read from reserved space if delta is inside
            auto result = find(needDeltaNum.begin(), needDeltaNum.end(), deltaId); //search deltaId
            if (result != needDeltaNum.end()) {
                debug_red("[DELTA] Reading from Reserve Segment ID = %" PRIu64 ",Block ID = %" PRIu32 ",Delta ID = %" PRIu32 "\n", segmentId, blockId, i);
                string deltaKey = generateDeltaKey (segmentId, blockId, deltaId);//segmentId.blockId.deltaId
                vector<offset_length_t> offsetLength = _deltaOffsetLength.get(deltaKey);
                uint32_t combinedLength = getCombinedLength(offsetLength);
                delta[i].info.segmentId = segmentId;
                delta[i].info.blockId = blockId;
                delta[i].info.blockSize = combinedLength; // size of delta (total)
                delta[i].info.offlenVector = offsetLength; //
                delta[i].info.blockIdOfDelta = deltaLocation.blockIdOfDelta;
                delta[i].buf = MemoryPool::getInstance().poolMalloc(combinedLength);
                memcpy (delta[i].buf, wholeBuf + deltaLocation.offsetLength.first, combinedLength);//read all space delta in buf
                i++;
                }
        }
        MemoryPool::getInstance().poolFree(wholeBuf);
        debug_cyan("[DELTA] get delta total number: %" PRIu32"\n",i);
        delta.erase(delta.begin()+i,delta.end());
    } else{
        debug_red(
                "[DELTA] NO Need to obtain delta in block %" PRIu32 "\n",blockId);
        delta.resize(delta.size()+1);
        delta[0].info.segmentId = segmentId;
        delta[0].info.blockId = blockId;
        delta[0].info.blockIdOfDelta = 999;// no need to obtain delta
        delta.erase(delta.begin()+1,delta.end());
    }
    return delta;
}
...
void StorageModule::mergeBlock (uint64_t segmentId, uint32_t blockId, bool isParity) {
    ...
    const string blockPath = generateBlockPath(segmentId, blockId, _blockFolder);
    uint32_t deltaCount = getDeltaCount(segmentId, blockId);
    debug_red ("segmentId: %" PRIu64 "Merge Block deltaCount = %" PRIu32 "\n", segmentId,deltaCount);
    if (deltaCount == 0) {
        return;
    }
    ...
    BlockData blockData = getMergedBlock(segmentId, blockId, isParity, false, symbols);
...
uint32_t StorageModule::writeDeltaBlock(uint64_t segmentId, uint32_t blockId,
                                        uint32_t deltaId,uint32_t blockIdOfDelta, char* buf, vector<offset_length_t> offsetLength, bool isParity) {
    ...
    if (isParity && combinedLength <= _reservedSpaceSize) {
        if (reserveSpaceInfo.remainingReserveSpace < combinedLength) {
            ...
            deltaKey = generateDeltaKey (segmentId, blockId, deltaLocation.deltaId);
            debug ("need merge remaining = %" PRIu32 " length = %" PRIu32 " deltaId = %" PRIu32 "\n", reserveSpaceInfo.remainingReserveSpace, combinedLength, deltaId);
            mergeBlock(segmentId, blockId, true);
            _deltaIdMap.set(blockKey,1);
#ifdef LATENCY_TEST
            typedef chrono::high_resolution_clock Clock;
            Clock::time_point t0 = Clock::now();
            cout << "MERGE TIME" << t0.time_since_epoch().count() << endl;
        ...
        deltaLocation.offsetLength = make_pair(currentOffset, combinedLength);
        deltaLocation.isReserveSpace = true;
        deltaLocation.blockIdOfDelta = blockIdOfDelta;
        debug ("block %" PRIu32 " written to reserve\n", blockId);
    } else {    // data block or delta too large
        createDeltaBlock(segmentId, blockId, deltaId, false);
        currentOffset = 0;
    ...
    debug_cyan(
            "Segment ID = %" PRIu64 " Block ID = %" PRIu32 " Delta ID = %" PRIu32 " write %" PRIu32 " bytes.\n",
            segmentId, blockId, deltaLocation.deltaId, byteWritten);
...
void StorageModule::createSegmentCache(uint64_t segmentId,uint32_t ifrom) {
        _segmentCache[segmentId] = ifrom;
}

uint32_t  StorageModule::getSegmentCache(uint64_t segmentId) {
    debug_red("getSegmentCache = %" PRIu64 " ,ifrom = %" PRIu32"  \n", segmentId,_segmentCache[segmentId]);
    if (!_segmentCache.count(segmentId)) {
        debug_red("segment cache not found for %" PRIu64 "\n", segmentId);
        _segmentCache[segmentId] = 0;
    }
...
int StorageModule::getgame(int *matrix,int blockID,int k ,int parityId) {
    int game;
    int i = parityId-k;
    game=matrix[blockID+k*i];
    return game;
}

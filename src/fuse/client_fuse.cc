...
static struct FileMetaData getAndCacheFileMetaData(uint32_t id) {
	...
	try {
		...
		for (uint32_t primary : fileMetaData._primaryList) {
		    // if at least one primary is disconnected, request latest metadata from Master
...
static void* ncvfs_init(struct fuse_conn_info *conn) {
	...
	_clientCommunicator->connectToMaster();
	_clientCommunicator->connectToMonitor();
	_clientCommunicator->getChunkserverListAndConnect();
	return NULL;
}

static int ncvfs_getattr(const char *path, struct stat *stbuf) {
	// Root Directory
	printf("\033[44;37;5m ncvfs_getattr\033[0m\n");
	if (strcmp(path, "/") == 0) {
	...
	if(retstat < 0) {
        // We print the error message into log in debug mode, mainly for 
        // recording and debugging if needed
        // (Note the error "No such file or directory" will always appear 
        // before file creation, since VFS always checks the existance of 
        // a file and truncates it if necessary)
		//perror("lstat()");
		debug("lstat(): %s\n", strerror(errno));
		// return error code to VFS 
		return -ENOENT;
	}
	...
	if (fileId == 0) {
		...
		//In Case the Record on Master is Deleted
		if(fileMetaData._fileType == NOTFOUND) {
			return -ENOENT;
		}
...
static int ncvfs_create(const char * path, mode_t mode, struct fuse_file_info *fi) {
	string fpath = _fuseFolder + string(path);
	printf("\033[44;37;5m ncvfs_create\033[0m\n");
...
static int ncvfs_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	printf("\033[44;37;5m ncvfs_read\033[0m\n");
	...
	while (sizeRead < size) {
		...
		// return immediately if data is cached, otherwise retrieve data from CHUNKSERVERs
...
static int ncvfs_write(const char *path, const char *buf, size_t size,
	...
	uint32_t ifrom = 999;
	char s = x[x.length()-1];
	char si = x[x.length()-2];
	if(si =='-'){
		ifrom = s - '0';
	}
	printf("\033[44;37;5m ncvfs_write\033[0m\n");
	struct FileMetaData fileMetaData = getAndCacheFileMetaData(fileId);
	...
	while (sizeWritten < size) {
		...
		uint32_t retstat = _fileDataCache->writeDataCache(segmentId, primary, bufptr, writeSize, segmentOffset, ifrom,fileMetaData._fileType);
		bufptr += retstat;
		sizeWritten += retstat;
	}
...
static int ncvfs_flush(const char *path, struct fuse_file_info *fi) {
	...
	printf("\033[44;37;5m ncvfs_flush\033[0m\n");
...
int ncvfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
		off_t offset, struct fuse_file_info *fi) {
	printf("\033[44;37;5m ncvfs_readdir\033[0m\n");
	DIR* dp;
	struct dirent *de;
...
static int ncvfs_release(const char* path, struct fuse_file_info *fi) {
	...
	printf("\033[44;37;5m ncvfs_release\033[0m\n");
	ncvfs_flush(path,fi);
